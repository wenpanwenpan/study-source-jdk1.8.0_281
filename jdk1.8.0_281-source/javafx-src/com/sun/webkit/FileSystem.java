...
final class FileSystem {
    ...
    private static RandomAccessFile fwkOpenFile(String path, String mode) {
        try {
            ...
            logger.log(Level.FINE, format("Error while creating "
                    + "RandomAccessFile for file [%s]", path), ex);
        }
        return null;
    }
    ...
    private static void fwkCloseFile(RandomAccessFile raf) {
        try {
            ...
            logger.log(Level.FINE, format("Error while closing "
                    + "RandomAccessFile for file [%s]", raf), ex);
        }
    }
    ...
    private static int fwkReadFromFile(RandomAccessFile raf, ByteBuffer byteBuffer) {
        try {
            ...
            logger.log(Level.FINE, format("Error while reading "
                    + "RandomAccessFile for file [%s]", raf), ex);
        }
        return -1;
    }
    ...
    private static void fwkSeekFile(RandomAccessFile raf, long pos) {
        try {
            ...
            logger.log(Level.FINE, format("Error while seek "
                    + "RandomAccessFile for file [%s]", raf), ex);
        }
    }
