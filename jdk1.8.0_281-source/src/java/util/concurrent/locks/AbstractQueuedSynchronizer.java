...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
        ...
        /**
         * 下面几个int值表示节点的状态，也就是 waitStatus 可能的值
         */
        /** waitStatus value to indicate thread has cancelled */
        // 线程被取消
        static final int CANCELLED =  1;
        /** waitStatus value to indicate successor's thread needs unparking */
        // 线程处于正常需要被唤醒的状态，当一个线程节点释放了锁，会唤醒他的下一个节点，改变waitStatus的值
         ...
         * 节点的等待状态：
         *  当前节点的线程的状态，标识该节点是否是正常节点，已取消，已挂起等
         */
     ...
     * 当state为0，则表明该锁不被任何线程持有
     * 当state为1，线程恰好持有该锁一次（未被重入）
     * 当state > 1，标识锁被线程重入 state 次
     */
    ...
    private Node enq(final Node node) {
        for (;;) {
            // 定义尾部临时节点
            Node t = tail;
            // 首节点的情况（即该链表上还没有元素，此时tail和head都是指向的null）
            if (t == null) { // Must initialize
                // 创建一个新的头节点，并通过cas的方式设置上去
                if (compareAndSetHead(new Node()))
                    // 如果setHead成功，则将尾节点也指向头节点（该节点为链表上的第一个节点，所以头尾节点指向一个节点）
                    tail = head;
            } else {
                // 如果尾节点不为空，则将当前节点的pre指向尾节点
                node.prev = t;
                // 通过cas将尾节点的next指向node节点
     ...
     *
     * @param node the node
     * 设置头节点，并且该头节点所代表的线程置为空，node.thread = null;
     */
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
             ...
             */
            // 否则，将前一个节点的waitStatus设置为SIGNAL（SIGNAL=-1，代表正常状态）
